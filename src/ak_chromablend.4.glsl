...
void main(void) {
  ...
  for(int x = -blur_value_int; x <= blur_value_int; x++) {
    ...
    vec3 sample  = adsk_rgb2yuv(texture2D(Back,current).rgb);
  ...
  if(energy > 0.0) {
    gl_FragColor = vec4(adsk_rgb2yuv(texture2D(Back,uv).rgb).r,(col/energy).g,(col/energy).b,1.0);
  } else {
    gl_FragColor = vec4(adsk_rgb2yuv(texture2D(Back,uv).rgb),1.0);
  }
}
